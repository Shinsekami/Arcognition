document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ---------- API endpoints ---------- */
  const SUPABASE_BASE =
    "https://kwyictzrlgvuqtbxsxgz.supabase.co/functions/v1";
  ...
  async function downloadToBase64(url) {
    log('Proxy download', url);
    const res = await fetchJson(DOWNLOAD_IMAGE_API, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ url }),
    });
    if (!res.base64) throw new Error('Image download failed');
    return res.base64;
  }

  async function detect(base64, width, height) {
    log('Calling detect API');
    const data = await fetchJson(DETECT_API, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ base64 }),
    });
    const anns = data.annotations || [];
    log('Detection success, annotations', anns.length);
    return anns.map((a) => {
      if (a.bbox) return a;
      ...
      return {
        ...
        bbox: {
          ...
          w: Math.round((xMax - xMin) * width),
          h: Math.round((yMax - yMin) * height),
        },
      };
    });
  }
  ...
  processBtn.addEventListener("click", async e => {
    ...
    try {
      ...
      // 2. detect objects
      const annotations = await detect(base64, preview.naturalWidth, preview.naturalHeight);
